...
export async function POST(request) {
  try {
    ...
    if (
      ...
      slug.includes("strategy-call") ||
      slug.includes("quiz-review") ||
      slug.includes("bullmania-cycle-review-session") ||
      slug.includes("bullmania-invitation-ai-included-deal")
    ) {
    ...
    if (assignedEmail) {
      ...
      // Filter: Must include "mechanical rules" (not review), OR "strategy call", OR "quiz review", OR "bullmania cycle review session", OR "bullmania invitation"
      const shouldRunTagging =
        ...
        eventNameLower.includes("bullmania invitation");
      ...
      if (shouldRunTagging) {
        ...
        } else if (assignedEmail === "ryan@bullmania.com") {
          tagIdToAssign = 22498075; // Ryan's tag
        } else if (assignedEmail === "eli@bullmania.com") {
          tagIdToAssign = 23068504; // Eli's tag
        }
